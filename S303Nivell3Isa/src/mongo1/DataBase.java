...
public class DataBase {
	...
	private MongoCollection coleccionStock;

	public DataBase() {

		this.mongoClient = MongoClients.create("mongodb://localhost:27017");
		this.database = mongoClient.getDatabase("MiFloristeria2");
		this.coleccion = database.getCollection("Productes");
		this.coleccionTickets = database.getCollection("Tickets");
		this.coleccionStock = database.getCollection("Stock");
	...
	public void verStock() {
		...
		MongoCursor<Document> cursor = coleccionStock.find().iterator();
		System.out.println();
		while (cursor.hasNext()) {
			Document doc = cursor.next();
			System.out.println(doc.toJson());
	...
	public void insertarArbol(Arbol arbol) {
		...
		coleccionStock.insertOne(amigo);
	}
	...
	public void actualizarArbol(Arbol arbol) {
		...
		coleccionStock.updateOne(query, updateObject);
	...
	public void vender(Stock stock) {

		BasicDBObject query = new BasicDBObject();
		query.put("_id", stock.get_id());

		BasicDBObject newDocument = new BasicDBObject();
		newDocument.put("creado", stock.getCreado());
		BasicDBObject updateObject = new BasicDBObject();
		updateObject.put("$set", newDocument);

		coleccionTickets.updateOne(query, updateObject);

		retirarProducto(stock.get_id());
	...
	public void retirarProducto(int _id) {
		...
		coleccionStock.deleteOne(new Document("_id", _id));
